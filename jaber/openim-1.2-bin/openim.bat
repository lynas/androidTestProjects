...
:runOpenIM
set RUN_CMD=%MERLIN_HOME%\bin\merlin.bat %OPENIM_HOME%\repository\openim\jars\openim-server-impl-1.2.jar -config file:/%OPENIM_HOME%\conf\config.xml -context %OPENIM_HOME% -kernel file:/%OPENIM_HOME%\conf\kernel.xml  %OPENIM_CMD_LINE_ARGS%
echo RUN CMD IS: %RUN_CMD%
call %RUN_CMD%
goto mainEnd

:runOpenIMNoHome
set OPENIM_HOME=c:/works/openim-1.2-bin
